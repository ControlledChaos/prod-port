	...
	public function update_messages( $messages ) {
		...
		foreach ( $post_types as $post_type => $post_object ) {
			...
			$messages[ $post_type ] = [
				...
				1  => sprintf(
					__( '%1s updated. <a href="%2s">View %3s</a>', 'prod-port' ), $post_object->labels->singular_name,
					esc_url( get_permalink( $post_ID ) ),
					$post_object->labels->singular_name
				),
				2  => __( 'Custom field updated.', 'prod-port' ),
				3  => __( 'Custom field deleted.', 'prod-port' ),
				4  => sprintf(
					__( '1%s updated.', 'prod-port' ),
					$post_object->labels->singular_name
				),
				5  => isset( $_GET['revision']) ? sprintf(
					__( '%1s restored to revision from %2s', 'prod-port' ),
					$post_object->labels->singular_name,
					wp_post_revision_title( (int) $_GET['revision'], false )
					) : false,
				6  => sprintf(
					__( '%1s published. <a href="%2s">View %3s</a>', 'prod-port' ),
					$post_object->labels->singular_name,
					esc_url( get_permalink( $post_ID ) ),
					$post_object->labels->singular_name
				),
				7  => sprintf(
					__( '%1s saved.', 'prod-port' ),
					$post_object->labels->singular_name
				),
				8  => sprintf(
					__( '%1s submitted. <a target="_blank" href="%2s">Preview %3s</a>', 'prod-port' ),
					$post_object->labels->singular_name,
				...
				9  => sprintf(
					__( '%1s scheduled for: <strong>%2s</strong>. <a target="_blank" href="%3s">Preview %4s</a>', 'prod-port'  ),
					$post_object->labels->singular_name,
					date_i18n( __( 'M j, Y @ G:i', 'prod-port' ),
					strtotime( $post->post_date ) ),
					esc_url( get_permalink( $post_ID ) ),
					$post_object->labels->singular_name
				),
				10 => sprintf(
					__( '%1s draft updated. <a target="_blank" href="%2s">Preview %3s</a>', 'prod-port'  ),
					$post_object->labels->singular_name,
					esc_url( add_query_arg( 'preview', 'true',
					get_permalink( $post_ID ) ) ),
					$post_object->labels->singular_name
 ...
 */
function ppp_type_taxe_functions() {
