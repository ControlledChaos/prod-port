...
if ( ppp_acf_options() ) {
    $sort_order = get_field( 'ppp_use_custom_sort_order', 'option' );

// If Advanced Custom Fields is not active.
} else {
    $sort_order = get_option( 'ppp_use_custom_sort_order' );
}
...
class Post_Types_Taxes_Order {
	...
	public static function instance() {
		...
		if ( is_null( $instance ) ) {
            ...
            if ( ! get_option( 'ppp_order_install ' ) ) {
    ...
    public function order_install() {
        ...
        update_option( 'ppp_order_install', 1 );
    ...
    public function admin_menu() {
        ...
        add_options_page(
            __( 'Posts & Taxonomies Sort Order', 'prod-port' ),
            __( 'Sort Order', 'prod-port' ),
            'manage_options',
            'sort-order-settings',
            [ $this, 'admin_page' ]
        );
    ...
    public function admin_page() {
        ...
        require PPP_PATH . 'admin/partials/settings-page-posts-order.php';
    ...
    public function load_script_css() {
        ...
        if ( $this->_check_load_script_css() ) {
            ...
            wp_enqueue_script( 'post-tax-order', PPP_URL . 'admin/assets/js/post-tax-order.js', [ 'jquery' ], null, true );
    ...
    public function update_options() {
        ...
        if ( ! isset( $_POST['ppp_posts_order_submit'] ) ) {
            return false;
        }

        check_admin_referer( 'ppp_posts_order_nonce' );

        $input_options            = [];
        $input_options['objects'] = isset( $_POST['objects'] ) ? $_POST['objects'] : '';
        $input_options['tags']    = isset( $_POST['tags'] ) ? $_POST['tags'] : '';

        update_option( 'ppp_order_options', $input_options );
    ...
    public function get_order_options_objects() {
        ...
        if ( $ppp_order_options = get_option( 'ppp_order_options' ) ) {
            $ppp_order_options = get_option( 'ppp_order_options' );
        } else {
            $ppp_order_options = [];
        }

        if ( isset( $ppp_order_options['objects'] ) && is_array( $ppp_order_options['objects'] ) ) {
            $objects = $ppp_order_options['objects'];
        } else {
            $objects = [];
        }
    ...
    public function get_order_options_tags() {
        ...
        if ( isset( $ppp_order_options['tags'] ) && is_array( $ppp_order_options['tags'] ) ) {
            $tags = $ppp_order_options['tags'];
        } else {
            $tags = [];
        }
 ...
 */
function ppp_post_types_taxes_order() {
...
function uninstall_db() {
    ...
    delete_option( 'ppp_order_install' );
