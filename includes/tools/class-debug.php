...
if ( ! defined( 'DOING_AJAX' ) && ! defined( 'DOING_AUTOSAVE' ) ) {
    add_action( 'admin_init', 'ppp_check_wp_debug_define' );
}

function ppp_check_wp_debug_define() {
	if ( defined( 'WP_DEBUG' ) && ! WP_DEBUG ) {
		$dev_mode = get_option( 'ppp_site_development' );
		if ( $dev_mode ) {
			ppp_set_wp_debug_define( true );
		} elseif ( false == $dev_mode )  {
			ppp_set_wp_debug_define( false );
		}
	}
}
 ...
 * @return void
 */
function ppp_set_wp_debug_define( $turn_it_on ) {
	...
	$config_file_path = ppp_find_wpconfig_path();
    if ( ! $config_file_path ) {
		return;
    }
	...
	*/
	apply_filters( 'ppp_set_wp_debug_define', $turn_it_on );
 ...
 */
function ppp_find_wpconfig_path() {
 ...
 * @return string|bool The path of wp-config.php file or false
 */
register_deactivation_hook( __FILE__, 'ppp_deactivation' );

function ppp_deactivation() {
    // set WP_DEBUG back to false
    set_ppp_wp_debug_off( false );
}

function set_ppp_wp_debug_off( $turn_it_off ) {
	// Get path of the config file
	$config_file_path = ppp_find_wpconfig_path();
    if ( ! $config_file_path ) {
		return;
	 ...
	 * @param string $turn_it_on The value of WP_DEBUG constant
	*/
	apply_filters( 'ppp_set_wp_debug_define', $turn_it_off );
