...
class Tools {
	...
	private function dependencies() {
		...
		$debug = get_option( 'ppp_debug_mode' );

		require_once PPP_PATH . 'includes/tools/class-debug.php';

		// Include the RTL (right to left) test if option selected.
		$rtl = get_option( 'ppp_rtl_test' );

		if ( $rtl ) {
			require_once PPP_PATH . 'includes/tools/class-rtl-test.php';
		}

		// Minify HTML source code.
		$minify = get_option( 'ppp_html_minify' );

		if ( $minify ) {
			require_once PPP_PATH . 'includes/tools/class-minify-process.php';
		}

		// Live theme test.
		$theme_test = get_option( 'ppp_theme_test' );

		if ( $theme_test ) {
			include_once PPP_PATH . 'includes/tools/class-theme-test.php';
		}
