...
class Media {
	...
	private function dependencies() {
		...
		include_once PPP_PATH . 'includes/media/class-svg-support.php';

		// Replace WP gallery shortcode if Fancybox option is used.
		$fancybox = get_option( 'ppp_enqueue_fancybox_script' );

		if ( $fancybox ) {
			require_once PPP_PATH . 'includes/media/class-gallery-shortcode.php';
		}
	...
	public function image_sizes() {
		...
		add_image_size( __( 'Meta Image', 'prod-port' ), 1200, 630, true );
		 ...
		 */
		add_image_size( __( 'Column Thumbnail', 'prod-port' ), 48, 48, true );
	...
	public function rss_featured_images( $content ) {
		...
		$size = apply_filters( 'ppp_rss_featured_image_size', 'medium' );
