...
class Meta_Title {
	...
	public function __construct() {
		...
		add_action( 'ppp_meta_title_tag', [ $this, 'title' ] );
	...
	public function title() {
		...
		$author_title = sprintf(
			'%1s %2s',
			__( 'Posts by', 'prod-port' ),
			get_the_author_meta( 'display_name', $author_id )
		);

		// Apply a filter to author archive title.
		$author_meta = apply_filters( 'ppp_author_meta_title', $author_title );

		// Custom search title.
		$search_title = sprintf(
			'%1s %2s',
			__( 'Searching', 'prod-port' ),
			get_bloginfo( 'name' )
		);

		// Apply a filter to search title.
		$search_meta = apply_filters( 'ppp_search_meta_title', $search_title );
