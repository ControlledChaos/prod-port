...
class Meta_Description {
	...
	public function __construct() {
		...
		add_action( 'ppp_meta_description_tag', [ $this, 'description' ] );
	...
	public function description() {
		...
		if ( ppp_acf_options() ) {
			 ...
			 */
			$acf_blog_desc = get_field( 'ppp_meta_blog_description', 'option' );
			...
			$wp_blog_desc = get_option( 'ppp_meta_blog_description' );
		...
		$search_desc      = wp_strip_all_tags(
			sprintf(
				'%1s \'%2s\'',
				__( 'Showing results for', 'prod-port' ),
				$search_query
			)
		);

		// Apply a filter to hard-coded text.
		$search_meta_desc = apply_filters( 'ppp_meta_description_search', $search_desc );
		...
		$meta_description = apply_filters( 'ppp_meta_description', $description );
