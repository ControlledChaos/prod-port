...
class Admin_Pages {
    ...
    public function __construct() {
        ...
        if ( ppp_acf_options() ) {
            ...
            $admin_header = get_field( 'ppp_use_admin_header', 'option' );
        } else {

            // Otherwise look for the WP API setting.
            $admin_header = get_option( 'ppp_use_admin_header' );
        }
    ...
    public function about_plugin() {
        ...
        if ( ppp_acf_options() ) {
            ...
            // Get the field.
            $link_label = get_field( 'ppp_site_plugin_link_label', 'option' );

        // If ACF is not active, get the field from the WordPress options page.
        } else {

            // Get the field.
            $link_label = sanitize_text_field( get_option( 'ppp_site_plugin_link_label' ) );
        }
        ...
        // Otherwise use Site Plugin as the label.
            ...
            $label = __( 'Site Plugin', 'prod-port' );
        }
        ...
        if ( ppp_acf_options() ) {
            ...
            // Get the field.
            $link_icon  = get_field( 'ppp_site_plugin_link_icon', 'option' );

        // If ACF is not active, get the field from the WordPress options page.
        } else {

            // Get the field.
            $link_icon  = sanitize_text_field( get_option( 'ppp_site_plugin_link_icon' ) );
        }
        ...
        // Otherwise use the scholar's cap icon to imply instruction.
            ...
            $icon = __( 'dashicons-welcome-learn-more', 'prod-port' );
        }
    ...
    public function about_plugin_output() {
        ...
        require PPP_PATH . 'admin/partials/plugin-page-about.php';
    ...
    public function help_about_plugin() {
		...
		$screen->add_help_tab( [
			'id'       => 'help_plugin_info',
			'title'    => __( 'More Information', 'prod-port' ),
			'content'  => null,
		...
		$screen->add_help_tab( [
			...
			'title'    => __( 'Convert Plugin', 'prod-port' ),
			'content'  => null,
	...
	public function help_plugin_info() {
		...
		include_once PPP_PATH . 'admin/partials/help/help-plugin-info.php';
	...
	public function help_convert_plugin() {
		...
		include_once PPP_PATH . 'admin/partials/help/help-plugin-convert.php';
    ...
    public function help_about_page_sidebar() {
        ...
        $html  = sprintf( '<h4>%1s</h4>', __( 'Author Credits', 'prod-port' ) );
        $html .= sprintf(
            '<p>%1s %2s.</p>',
            __( 'This plugin was originally written by', 'prod-port' ),
            'Greg Sweet'
        );
        $html .= sprintf(
            ...
            __( 'for more free downloads.', 'prod-port' )
        );
        $html .= sprintf(
            '<p>%1s</p>',
            __( 'Change this sidebar to give yourself credit for the hard work you did customizing this plugin.', 'prod-port' )
         );
	...
	public function admin_menus() {
		...
		register_nav_menus( [
			'admin-header' => __( 'Admin Header Menu', 'prod-port' )
		] );
	...
	public function admin_header() {
		...
		if ( ! empty( $admin_header ) ) {
			...
			include_once PPP_PATH . 'admin/partials/admin-header.php';
		}
    ...
    public function title_placeholders( $title ) {
        ...
        if ( 'post' == $screen->post_type ) {
            $post_title = esc_html__( 'Post Title', 'prod-port' );

        // Post type: page.
        } elseif ( 'page' == $screen->post_type ) {
            $post_title = esc_html__( 'Page Title', 'prod-port' );
        ...
        // Post type: custom, unidentified.
            ...
            $post_title = esc_html__( 'Enter Title', 'prod-port' );
        }

        // Apply a filter conditional modification.
        $title = apply_filters( 'ppp_post_title_placeholders', $post_title );
    ...
    private function get_column_image( $post_ID ) {
        ...
        $thumb = apply_filters( 'ppp_column_thumbnail_size', $size );
    ...
    public function image_column_head( $defaults ) {
        ...
        $name    = __( 'Featured Image', 'prod-port' );

        // Apply a filter for conditional modification.
        $heading = apply_filters( 'ppp_image_column_head', $name );
