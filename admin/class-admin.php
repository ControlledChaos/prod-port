...
class Admin {
	...
	private function dependencies() {
		...
		require_once PPP_PATH . 'admin/class-settings.php';

		// Add icons to the titles of ACF tab and accordion fields, if active.
		if ( class_exists( 'acf_pro' ) && ! get_option( 'ppp_acf_activate_settings_page' ) ) {
			include_once PPP_PATH . 'admin/class-acf-tab-icons.php';
		}

		// Include custom fields for Advanced Custom Fields Pro, if active.
		if ( ppp_acf() ) {
			include_once PPP_PATH . 'admin/class-settings-fields-site-acf.php';
		}

		// Functions for dasboard widgets, excluding the welcome panel.
		require_once PPP_PATH . 'admin/dashboard/class-dashboard.php';

		// Functions for admin menu item positions and visibility.
		require_once PPP_PATH . 'admin/class-admin-menu.php';

		// Add menus to the admin toolbar.
		require_once PPP_PATH . 'admin/class-admin-toolbar-menus.php';

		// Functions for various admin pages and edit screens.
		require_once PPP_PATH . 'admin/class-admin-pages.php';

		// Import custom fields for editing, if ACF Pro is active.
		if ( ppp_acf_options() ) {
			include_once PPP_PATH . 'admin/class-fields-import.php';
		}

		// Filter by page template.
		require_once PPP_PATH . 'admin/class-admin-template-filter.php';
	...
	public function admin_footer() {
		...
		if ( ppp_acf_options() ) {
			 ...
			 */
			$credit = get_field( 'ppp_admin_footer_credit', 'option' );
			$link   = get_field( 'ppp_admin_footer_link', 'option' );
			...
			if ( $credit && $link ) {
				$footer = sprintf(
					...
					$site,
					esc_html__( 'website designed & developed by', 'prod-port' ),
					esc_url( $link ),
					$credit
				);
			...
			// If no input we use the name of the site.
				...
				$footer = sprintf(
					'%1s %2s. ',
					$site,
					esc_html__( 'website powered by WordPress', 'prod-port' )
				);
			}
		 ...
		 */
			...
			$credit = sanitize_text_field( get_option( 'ppp_footer_credit' ) );
			$link   = esc_url_raw( get_option( 'ppp_footer_link' ) );
		...
		$admin_footer = apply_filters( 'ppp_admin_footer', $footer );
	...
	public function enqueue_styles() {
		 ...
		 * @since 1.0.0
		 */
		wp_enqueue_style( PPP_ADMIN_SLUG . '-admin', PPP_URL . 'admin/assets/css/admin.min.css', [], PPP_VERSION, 'all' );
		 ...
		 * @since 1.0.0
		 */
		wp_enqueue_style( PPP_ADMIN_SLUG . '-tooltips', PPP_URL . 'admin/assets/css/tooltips.min.css', [], PPP_VERSION, 'all' );
		 ...
		 */
		if ( ppp_acf() ) {
			wp_enqueue_style( PPP_ADMIN_SLUG . '-acf', PPP_URL . 'admin/assets/css/acf.css', [], PPP_VERSION, 'all' );
		}
		 ...
		 * @since 1.0.0
		 */
		$welcome = get_option( 'ppp_custom_welcome' );
		if ( $welcome ) {
			wp_enqueue_style( PPP_ADMIN_SLUG . '-welcome', PPP_URL . 'admin/assets/css/welcome.css', [], PPP_VERSION, 'all' );
		}
	...
	public function get_search_form() {
		...
		require PPP_PATH . 'partials/searchform.php';
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( PPP_ADMIN_SLUG . '-conditionalize', PPP_URL . 'admin/assets/js/admin.js', [ 'jquery' ], PPP_VERSION, true );

		// Enqueue scripts for backend functionality of this plugin.
		wp_enqueue_script( PPP_ADMIN_SLUG . '-admin', PPP_URL . 'admin/assets/js/conditionize.flexible.jquery.min.js', [ 'jquery' ], PPP_VERSION, true );
