...
class Welcome {
    ...
    public function __construct() {
		...
		if ( ppp_acf_options() ) {
			$dismiss = get_field( 'ppp_remove_welcome_dismiss', 'option' );

		// If ACF is not active, get the field from the WordPress options page.
		} else {
			$dismiss = get_option( 'ppp_remove_welcome_dismiss' );
		}
		...
		// If ACF is active, get the field from the ACF options page.
		if ( ppp_acf_options() ) {
			$welcome = get_field( 'ppp_custom_welcome', 'option' );
		} else {
	...
	public function widget_areas() {
		...
		register_sidebar( [
			'name'          => __( 'Welcome Panel - First Area', 'prod-port' ),
			'id'            => 'ppp_welcome_widget_first',
			'description'   => __( '', 'prod-port' ),
		...
		register_sidebar( [
			'name'          => __( 'Welcome Panel - Second Area', 'prod-port' ),
			'id'            => 'ppp_welcome_widget_second',
			'description'   => __( '', 'prod-port' ),
			'before_widget' => '<div id="%1$s" class="widget %2$s">',
			'after_widget'  => '</div>',
			'before_title'  => '<h3>',
		...
		register_sidebar( [
			'name'          => __( 'Welcome Panel - Third Area', 'prod-port' ),
			'id'            => 'ppp_welcome_widget_last',
			'description'   => __( '', 'prod-port' ),
			'before_widget' => '<div id="%1$s" class="widget %2$s">',
			'after_widget'  => '</div>',
			'before_title'  => '<h3>',
	...
	public function welcome_panel() {
		...
		if ( ! empty( $welcome ) ) {
			...
			include_once PPP_PATH . 'admin/dashboard/partials/welcome-panel.php';
		}
	...
	public function styles() {
        ...
        if ( $screen->id == 'dashboard' ) {
            wp_enqueue_style( PPP_ADMIN_SLUG . '-welcome', PPP_URL .  'assets/css/welcome.min.css', [], null, 'screen' );
        }
