...
class Meta_SEO_Callbacks {
	...
	public function disable_meta( $args ) {
		...
		$option = get_option( 'ppp_meta_disable' );

		$html = '<p><input type="checkbox" id="ppp_meta_disable" name="ppp_meta_disable" value="1" ' . checked( 1, $option, false ) . '/>';

		$html .= '<label for="ppp_meta_disable"> '  . $args[0] . '</label></p>';
	...
	public function  schema_org_type( $args ) {
		...
		// Get the field file.
		include_once PPP_PATH . 'admin/partials/field-callbacks/schema-org-type.php';
 ...
 */
function ppp_meta_seo_callbacks() {
