...
$settings_icon = sanitize_text_field( get_option( 'ppp_site_settings_link_icon' ) );
...
$tabs = [
    ...
    sprintf(
        ...
        '#intro',
        esc_html__( 'Introduction', 'prod-port' )
	),
    ...
    sprintf(
		...
		'#settings',
		esc_attr( $settings ),
        esc_html__( 'Site Settings', 'prod-port' )
	),
    ...
    sprintf(
        ...
        '#scripts',
        esc_html__( 'Script Options', 'prod-port' )
	),
    ...
    sprintf(
        ...
        '#media',
        esc_html__( 'Media Options', 'prod-port' )
	),
    ...
    sprintf(
        '<li><a href="%1s"><span class="dashicons dashicons-welcome-learn-more"></span> %2s</a></li>',
        '#tools',
        esc_html__( 'Development Tools', 'prod-port' )
    ),

];

// Apply a filter to the tabs array for adding tabs.
$page_tabs = apply_filters( 'ppp_tabs_page_about', $tabs );

?>
<!-- Default WordPress page wrapper -->
<div class="wrap site-plugin-wrap">
	<!-- Page heading -->
	<?php echo sprintf( '<h1 class="wp-heading-inline">%1s %2s</h1>', get_bloginfo( 'name' ), esc_html__( 'Plugin', 'prod-port' ) ); ?>
	<!-- Page description -->
    <p class="description"><?php esc_html_e( 'A feature-packed WordPress starter plugin for building custom-tailored websites.', 'prod-port' ); ?></p>
	<!-- Ornamental divider -->
	...
	<div class="backend-tabbed-content">
		...
		do_action( 'ppp_content_page_about_before' ); ?>
		<!-- Begin content -->
		<div id="intro"><!-- Introduction content -->
			<?php include_once PPP_PATH . 'admin/partials/plugin-page-intro.php'; ?>
		</div>
		<div id="settings"><!-- Site Settings content -->
			<?php include_once PPP_PATH . 'admin/partials/plugin-page-site-settings.php'; ?>
		</div>
		<div id="scripts"><!-- Script Options content -->
			<?php include_once PPP_PATH . 'admin/partials//plugin-page-script-options.php'; ?>
		</div>
		<div id="media"><!-- Media Options content -->
			<?php include_once PPP_PATH . 'admin/partials/plugin-page-media-options.php'; ?>
		</div>
		<div id="tools"><!-- Dev Tools content -->
			<?php include_once PPP_PATH . 'admin/partials/plugin-page-dev-tools.php'; ?>
		</div>
		<?php // Hook for adding tabbed content.
		do_action( 'ppp_content_page_about_after' ); ?>
