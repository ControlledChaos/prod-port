...
$tabs = [
    ...
    sprintf(
        ...
        esc_html__( 'Dashboard', 'prod-port' )
    ),
    ...
    sprintf(
        ...
        esc_html__( 'Admin Menu', 'prod-port' )
    ),
    ...
    sprintf(
        ...
        esc_html__( 'Admin Pages', 'prod-port' )
    ),
    ...
    sprintf(
        ...
        esc_html__( 'Site Users', 'prod-port' )
    ),
    ...
    sprintf(
        ...
        esc_html__( 'Meta/SEO', 'prod-port' )
    )

];

// Apply a filter to the tabs array for adding tabs.
$page_tabs = apply_filters( 'ppp_tabs_site_settings', $tabs );
...
if ( 'dashboard' == $active_tab  ) {
    $section = 'ppp-site-dashboard';
    $fields  = 'ppp-site-dashboard';
} elseif ( 'admin-menu' == $active_tab ) {
    $section = 'ppp-site-admin-menu';
    $fields  = 'ppp-site-admin-menu';
} elseif ( 'admin-pages' == $active_tab ) {
    $section = 'ppp-site-admin-pages';
    $fields  = 'ppp-site-admin-pages';
} elseif ( 'users' == $active_tab ) {
    $section = 'ppp-site-users';
    $fields  = 'ppp-site-users';
} elseif ( 'meta-seo' == $active_tab ) {
    $section = 'ppp-site-meta-seo';
    $fields  = 'ppp-site-meta-seo';
} else {
    $section = null;
    $fields  = null;
}
...
$do_section = apply_filters( 'ppp_section_site_settings', $section );
$do_fields  = apply_filters( 'ppp_fields_site_settings', $fields );
 ...
 */
if ( 'dashboard' == $active_tab  ) {
    $save = __( 'Save Dashboard', 'prod-port' );
} elseif ( 'admin-menu' == $active_tab ) {
    $save = __( 'Save Menu', 'prod-port' );
} elseif ( 'admin-pages' == $active_tab ) {
    $save = __( 'Save Pages', 'prod-port' );
} elseif ( 'users' == $active_tab ) {
    $save = __( 'Save Users', 'prod-port' );
} elseif ( 'meta-seo' == $active_tab ) {
    $save = __( 'Save Meta', 'prod-port' );
} else {
    $save = __( 'Save Settings', 'prod-port' );
}

// Apply a filter for new tabs added by another plugin or from a theme.
$button = apply_filters( 'ppp_save_site_settings', $save );
...
<div class="wrap">
	<?php echo sprintf(
        ...
        get_bloginfo( 'name' ),
        esc_html__( 'Settings', 'prod-port' )
    ); ?>
    <p class="description"><?php esc_html_e( 'Customize the way WordPress is used.', 'prod-port' ); ?></p>
