...
$tabs = [
    ...
    sprintf(
        ...
        esc_html__( 'Vendor', 'prod-port' )
    )

];

// Apply a filter to the tabs array for adding tabs.
$page_tabs = apply_filters( 'ppp_tabs_script_options', $tabs );
...
if ( 'general' == $active_tab ) {
    $section = 'ppp-scripts-general';
    $fields  = 'ppp-scripts-general';
} elseif ( 'vendor' == $active_tab ) {
    $section = 'ppp-scripts-vendor';
    $fields  = 'ppp-scripts-vendor';
} else {
    $section = null;
    $fields  = null;
}
...
$do_section = apply_filters( 'ppp_section_script_options', $section );
$do_fields  = apply_filters( 'ppp_fields_script_options', $fields );
...
?>
<div class="wrap">
    <h1 class="wp-heading-inline"><?php esc_html_e( 'Script Options', 'prod-port' ); ?></h1>
    <?php if ( is_rtl() ) : ?>
    <p class="description"><?php esc_html_e( 'Script settings from the Controlled Chaos plugin. More information in the Help tab at upper left.', 'prod-port' ); ?></p>
    <?php else : ?>
    <p class="description"><?php esc_html_e( 'Script settings from the Controlled Chaos plugin. More information in the Help tab at upper right.', 'prod-port' ); ?></p>
    <?php endif; ?>
