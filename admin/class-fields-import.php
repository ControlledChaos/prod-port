...
final class Fields_Import {
	...
	public function import_page() {
		...
		add_submenu_page(
			...
			__( 'Registered Fields', 'prod-port' ),
			__( 'Registered Fields', 'prod-port' ),
	...
	public function page_output() {
		...
		if ( isset( $_POST['acf_php_recovery_action'] ) && $_POST['acf_php_recovery_action'] == 'import' && isset( $_POST['fieldsets']) && check_admin_referer( 'acf_php_recovery' ) ) {
			...
			foreach ( $acf_local->groups as $key => $group ) {
				$group['title'] = $group['title'] . __( ' (Imported)', 'prod-port' );
		...
		include_once PPP_PATH . 'admin/partials/fields-import-page.php';
